...
//		this.log = LogManager.getLogger(GetAllCRNListComSD.class);
	}
...
//		Log4jLoggerFile.info("Hitting the GetCRNList API");
		RestAssured.baseURI = url;
		String response = given().queryParam("state_cd", "07").queryParam("start_tm", "2025-03-10:09:16")
				.queryParam("end_tm", "2025-03-10:10:16").queryParam("action", "CRNLST")
				.queryParam("casetyp", "ADJDT").queryParam("iseod", "Y")
				.headers(header).when().get("govtapi/v1.0/caselst").then().extract().asString();

//		Log4jLoggerFile.info("GetCRNList API response received.");
...
//		Log4jLoggerFile.info("Completed the_user_hits_GetCRNList_api method");
	}
}
