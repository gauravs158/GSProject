...
public class GetFileDTLSLedgerSD {
	...
	String status_cd;
	public Logger log;
	public GetFileDTLSLedgerSD(Heart heart) {
		this.heart = heart;
		this.url = heart.url;
		this.authToken = heart.govtAuthToken;
		this.log = LogManager.getLogger(GetFileDTLSLedgerSD.class);
	}

	@When("the user hits LedgerGetFileDetailsAPI")
	public void the_user_hits_GetFileDetails_api() {
		...
		Log4jLoggerFile.info("Hitting the Ledger GetFileDetails API");
		RestAssured.baseURI = url;
		String response = given().queryParam("state_cd", "21").queryParam("setlprd", "012025")
				.queryParam("rptnum", "STL_01_01").queryParam("action", "FILEDETL").queryParam("filno", "1")
				.headers(header).when().get("govtapi/v0.1/setlldg").then().extract().asString();

		Log4jLoggerFile.info("Ledger GetFileDetails API response received.");
		...
		Log4jLoggerFile.info("Completed the_user_hits_GetFileDetails_api method of Ledger Module");
	}
}
